...
\title[Appendix 2b]{Appendix 2b: The \texttt{lattice} graphics package}
\SweaveOpts{engine=R,eps=FALSE,pdf=TRUE,width=10,height=6.5,strip.white=all,keep.source=TRUE}
\SweaveOpts{prefix=TRUE,prefix.string=figs/ch02b,include=TRUE}
\begin{document}
\begin{frame}
  \titlepage
\end{frame}
...
@ 
  \begin{center}
<<xyplot,echo=FALSE,fig=TRUE,height=6>>=
