...
\begin{frame}
  ...
  \item \Emph{Yates' algorithm} is a method for determining these
    coefficient estimates by hand calculation.  It is interesting but
    no longer needed.

  \item Notice also that the standard errors of all the coefficients
    are identical.  This provides an alternative approach to
    evaluating the significance of coefficients in a \Emph{saturated}
...
@ 
\begin{frame}[fragile]
  \frametitle{QQ-plot of coefficients from the saturated model}
<<coeffm10>>=
(cc <- coef(fm10 <- lm(ok ~ A * B * C, dents))[-1])
@   
\begin{center}
<<probplt,fig=TRUE,echo=FALSE,height=5.5>>=
print(qqmath(~ cc, aspect = 1,
             xlab = "Standard normal quantiles",
             ylab = "Estimates coefficients",
             panel = function(...){
                 panel.grid(h = -1, v = -1)
                 panel.qqmath(...)
                 panel.qqmathline(...)
...
\end{frame}
\begin{frame}
  \frametitle{Interpretation of the QQ plot of coefficient estimates}
  \begin{itemize}
  \item The plot on the previous slide indicates that 2 coefficients
  ...
  \item Generally this approach is preferred to the approach of
  ...
  \item One variation on this approach is to compare the absolute
    value of the coefficients to the theoretical quantiles.  This is
    called a ``half-normal'' plot.
  \item It is not part of the course but the way to create such a plot
...
\end{frame}
\begin{frame}[fragile]
  \frametitle{Half-normal plot of saturated model coefficients}
  \begin{center}
<<halfnormal,fig=TRUE,height=5>>=
...
\begin{frame}
  \frametitle{Section 9.3 Fractional factorial designs}
  \begin{itemize}
  \item 
  \end{itemize}
\end{frame}
...
<<xmp932>>=
xmp932 <- data.frame(y = c(3.6,10,8,3.2,7.6,3.2,3.7,6.0),
                     A = gl(2,1,8,ord=1), B = gl(2,2,8,ord=1),
                     C = gl(2,4,ord=1))
@   
\end{frame}
